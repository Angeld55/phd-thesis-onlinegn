\subsubsection{Примерна имплементация на алгоритъма на Javascript}
В следващия листинг е показан реалният JavaScript, който реализира описания алгоритъм. Той включва две основни функции: \texttt{convertEdgesToML}, която нормализира \texttt{path} описанията на ръбовете, и \texttt{svgToTeX}, която извлича всички места, преходи и ръбове и генерира финалния \TeX{} код.

\begin{verbatim}
/**
 * normalize "path" elements in edges from e.g. "M95.5,-90 L119.12,-90"
 */
function convertEdgesToML(svgString) {
...
function svgToTeX(
  ...
  const LENGTH_SCALE_FACTOR = 6/10;  // <-- key factor: shrink geometry to 2/3
  ...
  // -----------------------
  function fixCircleBoundary(cx, cy, r, oldPt) {
    const dx = oldPt.x - cx;
    const dy = oldPt.y - cy;
  ...
  function fixRectBoundary(xMin, xMax, yMin, yMax, oldPt, neighborPt) {
    ...
    if (shapeA) {
      ...
      if (shapeA.type === "circle") {
        ...
        rawPoints[0] = fixRectBoundary(
          shapeA.xMin, shapeA.xMax, shapeA.yMin, shapeA.yMax,
          p0, p1
        );
      }
    }
    ...
    if (shapeB) {
      ...
      if (shapeB.type === "circle") {
        ...
        newLast = fixRectBoundary(
          shapeB.xMin, shapeB.xMax, shapeB.yMin, shapeB.yMax,
          pLast, pBefore
        );
      }
  ...
  edges.forEach(e => {
    e.rawPoints.forEach(pt => {
      allX.push(pt.x);
      allY.push(pt.y);
    });
    e.arrowPoly.forEach(pt => {
      allX.push(pt.x);
      allY.push(pt.y);
    });
  });
  ...
  edges.forEach(e => {
    ...
    const Tpoints = rawPoints.map(pt => toTeXCoords(pt.x, pt.y));
